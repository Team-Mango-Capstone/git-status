...
export async function searchRepos(user) {
  try {
    const { data } = await axios.get(
      `https://api.github.com/search/repositories?q=user:${user}}+fork:true+archived:false&per_page=100 `
    );
...
export async function searchCommits(user, repo) {
  try {
    ...
    const { data } = await axios.get(
      `https://api.github.com/search/commits?q=author:${user} repo:${repo} merge:false fork:true sort:author-date`
    );
    return data.items;
  } catch (err) {
    console.log('error');
